// Given a string, represented as a character array, design a recursive algorithm to replace all the pi's in the given string with 3.14 & print the resulting string. 
...
void replacePi(char* inp, int i) {
    ...
    if(inp[i] == 'p' and inp[i+1] == 'i') {
        ...
        while(j >= i+2) {
            // shift all the char at the jth index two-steps to the right
            inp[j+2] = inp[j];
            j--;
        }
