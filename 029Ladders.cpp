/*
Suppose there is a ladder with n steps. A person standing at the bottom of the ladder and wants to reach to its top. Assume the person can take atmost k steps at a time. Then, count the number of ways, the person can reach the top of the ladder.
Example 
	Input : n = 4, k = 3
	Output: 7
	Explanation : [1, 1, 1, 1], 
...
int countNumberOfWays(int n, int k, int i, string out) {
    ...
    for(int j=1; j<=k; j++) {
        ...
        // out += to_string(j);
        // count += countNumberOfWays(n, k, i+j, out);
        // out.pop_back(); // back_tracking step
